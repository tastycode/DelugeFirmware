#! /usr/bin/env python3
from dbt.util import resolve_input_device, resolve_output_device, unpack_7bit_to_8bit
...
def load_sysex(args) -> None:
    ...
    if not loadfw_path.exists():
        compile_command = ["gcc", "./contrib/load/loadfw.c", "src/deluge/util/pack.c", "-Isrc/deluge", "-Icontrib/load", "-o", str(loadfw_path)]
        run_command(compile_command, args.verbose)


    input_device = resolve_input_device(args.input)
    output_device = resolve_output_device(args.output)
    source_build = resolve_source_build(args.build)
    # Invoke loadfw with the specified arguments
    loadfw_command = [str(loadfw_path), "-o", "output.syx", args.key, source_build]
    print(f"Running command: {' '.join(loadfw_command)}")
    run_command(loadfw_command, args.verbose)

    # Use mido to send the syx file to the device
    print(f"Sending syx file to {output_device}")

    MAX_KB_SEC = 300
    bytes_sent_sec = 0
    message_index = {}
    with mido.open_input(input_device) as input:
        with mido.open_output(output_device) as output:
            for msg in tqdm(mido.read_syx_file("output.syx")):
                cmd = msg.bytes()[3]
                message_index.setdefault(cmd, [])
                message_index[cmd].append(msg)
            ...
            for message in input:
                ...
                if message_bytes[0:4] == [0xf0, 0x7d, 0x03, 0x06]:
                    failed_count = message_bytes[5]
                    i = 0
                    while failed_count > 0:
                        packed_seg = message_bytes[(5 + i):(5+i+4)]
                        unpacked_seg = unpack_7bit_to_8bit(packed_seg)
                        seg = int.from_bytes(unpacked_seg, byteorder='little', signed=False)  # Use 'little' or 'big' depending on endianness
                        msg = message_index[1][seg]
                        output.send(msg)
                        print(f"Failed seg {seg} resent")
                        failed_count -= 1
                        i += 1
                    validate_msg = message_index[2][0]
                    output.send(validate_msg)

            # if the 4th byte is 0x06
            print(f"received message after send {message.hex()}")
        print(f"Sent syx file: {source_build} to device {output_device}")
